...
test("正常系: マージ方式を指定したとき その方式だけを許可する設定になる", () => {
  const payload = makeRepositorySettingsPayload({
    currentDefaultBranch: "main",
    selectedBranch: "main",
    mergeMethod: "squash"
...
test("準正常系: マージ方式を指定しないとき マージ方式は変更しない", () => {
  const payload = makeRepositorySettingsPayload({
    currentDefaultBranch: "main",
    selectedBranch: "main"
  });
...
test("正常系: マージ方式を指定したとき 通常実行の PATCH に許可設定を含める", async () => {
  const apiCalls = [];
  const deps = createSetupDeps({
    apiCalls,
    ghApi: (endpoint, options = {}) => {
...
test("準正常系: 単体適用とマージ方式を同時に指定したとき エラーになる", () => {
  assert.throws(
    () => parseArgs(["--only", "delete-branch-on-merge", "--merge-method", "squash"]),
    /--merge-method cannot be used with --only delete-branch-on-merge\./
  );
});
