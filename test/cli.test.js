...
test("準正常系: 直コミット許可を選んだとき branch protection 作成は選ばない", () => {
  const selection = resolveSelection(parseArgs(["--repo", "owner/repo", "--branch", "develop", "--allow-direct-commits"]));
...
test("異常系: branch を指定せずに直コミット許可を選んだとき エラーになる", () => {
  assert.throws(
    () => parseArgs(["--repo", "owner/repo", "--allow-direct-commits"]),
    /--allow-direct-commits requires --branch/
  );
});

test("異常系: 直コミット許可と approval 数を同時に指定したとき エラーになる", () => {
  assert.throws(
    () => parseArgs(["--repo", "owner/repo", "--branch", "main", "--allow-direct-commits", "--required-approvals", "1"]),
    /--required-approvals cannot be used with --allow-direct-commits/
  );
});

test("準正常系: ブランチ削除だけを選んだとき delete_branch_on_merge だけを含む", () => {
  const payload = makeRepositorySettingsPayload({
    selection: { branchProtection: false, allowDirectCommits: false, deleteBranchOnMerge: true, mergeMethod: false }
  });
...
test("正常系: branch protection で default branch を変更するとき default_branch を含む", () => {
  const payload = makeRepositorySettingsPayload({
    selection: { branchProtection: true, allowDirectCommits: false, deleteBranchOnMerge: true, mergeMethod: false },
...
test("正常系: マージ方式だけを選んだとき 許可設定だけを含む", () => {
  const payload = makeRepositorySettingsPayload({
    selection: { branchProtection: false, allowDirectCommits: false, deleteBranchOnMerge: false, mergeMethod: true },
    mergeMethod: "squash"
  });
...
test("準正常系: 直コミット許可のとき PR 必須だけを外して既存 ruleset を更新する", async () => {
  const apiCalls = [];
  const deps = createSetupDeps({
    apiCalls,
    ghApi: (endpoint, options = {}) => {
      apiCalls.push({ endpoint, options });
      if (endpoint === "/repos/owner/repo/rulesets/123" && options.method === undefined) {
        return {
          name: "Require PR to main",
          target: "branch",
          enforcement: "active",
          conditions: {
            ref_name: {
    ...
    findExistingRuleset: () => ({ id: 123, name: "Require PR to main" })
  });
  const args = parseArgs(["--repo", "owner/repo", "--branch", "main", "--allow-direct-commits", "--yes"]);

  await runSetup(args, deps);

  assert.deepEqual(apiCalls.map((call) => [call.endpoint, call.options.method ?? "GET"]), [
    ["/repos/owner/repo/rulesets/123", "GET"],
    ["/repos/owner/repo/rulesets/123", "PUT"]
  ]);
...
test("異常系: 直コミット許可の対象 ruleset がないとき エラーになる", async () => {
  const deps = createSetupDeps({
    findExistingRuleset: () => null
  });
  const args = parseArgs(["--repo", "owner/repo", "--branch", "main", "--allow-direct-commits", "--yes"]);
  ...
  await assert.rejects(
    () => runSetup(args, deps),
    /Ruleset 'Require PR to main' was not found/
  );
});

test("異常系: 指定 branch と違う ruleset のとき PR 必須を外さない", async () => {
  const apiCalls = [];
  const deps = createSetupDeps({
    apiCalls,
    ghApi: (endpoint, options = {}) => {
    ...
    findExistingRuleset: () => ({ id: 123, name: "shared ruleset" })
  });
